...
function parseTokens() {
	return tsModule === null && (tsModule = createRequire(import.meta.url)("./typescript.cjs"), tsSyntaxKind = tsModule.SyntaxKind), convertTokens(tsModule.createSourceFile(filePath, sourceText, {
	...
	}, !0, tsModule.ScriptKind.TSX));
}
function convertTokens(tsAst) {
	let tokens$1 = [];
	return walk(tsAst), tokens$1;
	function walk(node) {
		let { kind } = node;
		kind === tsSyntaxKind.SingleLineCommentTrivia || kind === tsSyntaxKind.MultiLineCommentTrivia || kind === tsSyntaxKind.JSDoc || (kind >= tsSyntaxKind.FirstToken && kind <= tsSyntaxKind.LastToken && kind !== tsSyntaxKind.EndOfFileToken ? convertToken(node, tsAst, tokens$1) : node.getChildren(tsAst).forEach(walk));
	}
}
function convertToken(token, tsAst, tokens$1) {
	...
	if (kind === tsSyntaxKind.RegularExpressionLiteral) tokens$1.push({
		...
		tokenType === "PrivateIdentifier" && (value = value.slice(1)), tokens$1.push({
			__proto__: TokenProto,
			type: tokenType,
			value,
			start,
...
function getTokenType(token) {
	...
	switch (kind) {
		...
		case tsSyntaxKind.RegularExpressionLiteral: return "RegularExpression";
		case tsSyntaxKind.Identifier: {
			let { parent: parent$1 } = token, parentKind = parent$1.kind;
			if (isJSXTokenKind(parentKind) || parentKind === tsSyntaxKind.PropertyAccessExpression && hasJSXAncestor(parent$1.parent)) return "JSXIdentifier";
		}
	}
	return "Identifier";
}
...
function deserializeProgramOnly(buffer$1, sourceText$2, sourceByteLen$2, getLoc$2) {
	return deserializeWith(buffer$1, sourceText$2, sourceByteLen$2, getLoc$2, deserializeProgram);
}
...
function deserializeProgram(pos) {
	let localAstId = astId, end = deserializeU32(pos + 4), program = parent = {
		...
		get comments() {
			...
			let comments$1 = deserializeVecComment(pos + 24), { hashbang } = this;
			if (hashbang !== null) {
				let start, end$1;
				comments$1.unshift({
					__proto__: NodeProto,
...
function deserializeObjectAssignmentTarget(pos) {
	let start = deserializeU32(pos), end = deserializeU32(pos + 4), previousParent = parent, node = parent = {
		...
		range: [start, end],
		parent
	}, properties$1 = deserializeVecAssignmentTargetProperty(pos + 8), rest = deserializeOptionBoxAssignmentTargetRest(pos + 32);
	return rest !== null && properties$1.push(rest), node.decorators = [], node.properties = properties$1, node.optional = !1, parent = previousParent, node;
}
...
function deserializeObjectPattern(pos) {
	let start = deserializeU32(pos), end = deserializeU32(pos + 4), previousParent = parent, node = parent = {
		...
		range: [start, end],
		parent
	}, properties$1 = deserializeVecBindingProperty(pos + 8), rest = deserializeOptionBoxBindingRestElement(pos + 32);
	return rest !== null && properties$1.push(rest), node.decorators = [], node.properties = properties$1, node.optional = !1, parent = previousParent, node;
}
...
function deserializeTSModuleDeclaration(pos) {
	...
	if (body === null) node = parent = {
		...
		let id$1 = deserializeTSModuleDeclarationName(pos + 8);
		if (body.type === "TSModuleBlock") node.id = id$1, body.parent = node;
		else {
			let innerId = body.id;
			if (innerId.type === "Identifier") {
				let start$1, end$1, outerId = node.id = parent = {
					__proto__: NodeProto,
					type: "TSQualifiedName",
					left: id$1,
					right: innerId,
					start: start$1 = id$1.start,
					end: end$1 = innerId.end,
					range: [start$1, end$1],
					parent: node
				};
				id$1.parent = innerId.parent = outerId;
			} else {
				node.id = innerId, innerId.parent = node;
				let { start: start$1 } = id$1;
				for (; innerId.start = innerId.range[0] = start$1, innerId.left.type !== "Identifier";) innerId = innerId.left;
				let end$1, right = innerId.left;
				id$1.parent = right.parent = innerId.left = {
					__proto__: NodeProto,
					type: "TSQualifiedName",
					left: id$1,
					right,
...
function deserializeComment(pos) {
	let type$1 = deserializeCommentKind(pos + 12), start = deserializeU32(pos), end = deserializeU32(pos + 4);
	return {
		__proto__: NodeProto,
		type: type$1,
		value: sourceText$1.slice(start + 2, end - (type$1 === "Line" ? 0 : 2)),
		start,
		end,
		range: [start, end]
	};
...
var require_eslint_visitor_keys = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	for (let type$1 of NODE_TYPES) Object.freeze(KEYS[type$1]);
	Object.freeze(KEYS);
	...
	function unionWith(additionalKeys) {
		...
		for (let type$1 of Object.keys(additionalKeys)) if (Object.hasOwn(retv, type$1)) {
			let keys = new Set(additionalKeys[type$1]);
			for (let key of retv[type$1]) keys.add(key);
			retv[type$1] = Object.freeze(Array.from(keys));
		} else retv[type$1] = Object.freeze(Array.from(additionalKeys[type$1]));
	...
	var __createBinding = exports && exports.__createBinding || (Object.create ? (function(o, m, k, k2) {
		...
		var ownKeys = function(o) {
			return ownKeys = Object.getOwnPropertyNames || function(o$1) {
				var ar = [];
...
})), require_lib$5 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	}), __exportStar(require_lib$5(), exports), __exportStar(require_parser_options(), exports), __exportStar(require_ts_estree(), exports);
	...
	var DefinitionBase = class {
		...
		node;
		parent;
		constructor(type$1, name, node, parent$1) {
			this.type = type$1, this.name = name, this.node = node, this.parent = parent$1;
		}
	};
...
})), require_lib$4 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	let types_1 = require_dist$1(), assert_1 = require_assert$1(), definition_1 = require_definition(), ID_1 = require_ID(), Reference_1 = require_Reference(), variable_1 = require_variable(), ScopeType_1 = require_ScopeType(), ImplicitLibVariable_1 = require_ImplicitLibVariable();
	...
	exports.ImportVisitor = class ImportVisitor extends Visitor_1.Visitor {
		...
		visitImport(id$1, specifier) {
			this.#referencer.currentScope().defineIdentifier(id$1, new definition_1.ImportBindingDefinition(id$1, specifier, this.#declaration));
		}
	};
	...
	let types_1 = require_dist$1(), assert_1 = require_assert$1(), definition_1 = require_definition(), lib_1 = require_lib$3(), ClassVisitor_1 = require_ClassVisitor(), ExportVisitor_1 = require_ExportVisitor(), ImportVisitor_1 = require_ImportVisitor(), PatternVisitor_1 = require_PatternVisitor(), Reference_1 = require_Reference(), TypeVisitor_1 = require_TypeVisitor(), Visitor_1 = require_Visitor();
...
function report(diagnostic, ruleDetails) {
	...
	if (ObjectHasOwn(diagnostic, "loc") && (loc = diagnostic.loc) != null) {
		...
		ObjectHasOwn(loc, "start") ? (start = getOffsetFromLineColumn(loc.start), end = loc.end == null ? start : getOffsetFromLineColumn(loc.end)) : (start = getOffsetFromLineColumn(loc), end = start);
	} else {
		let { node } = diagnostic;
...
const EMPTY_GLOBALS = ObjectFreeze({});
let globalsJSON = null, globals = null;
function setGlobalsForFile(globalsJSONInput) {
	globalsJSON = globalsJSONInput;
}
function initGlobals() {
	globalsJSON === "{}" ? globals = EMPTY_GLOBALS : (globals = JSONParse(globalsJSON), ObjectFreeze(globals));
}
...
}), PARSER_OPTIONS = ObjectFreeze({ get sourceType() {
...
var require_uri_all = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	(function(global, factory) {
		...
		function buildExps(isIRI) {
			...
			return subexp(URI$ + "|" + RELATIVE$), subexp(SCHEME$ + "\\:" + HIER_PART$ + subexp("\\?" + QUERY$) + "?"), "" + SCHEME$ + subexp(subexp("\\/\\/(" + subexp("(" + USERINFO$ + ")@") + "?(" + HOST$ + ")" + subexp("\\:(" + PORT$ + ")") + "?)") + "?(" + PATH_ABEMPTY$ + "|" + PATH_ABSOLUTE$ + "|" + PATH_ROOTLESS$ + "|" + PATH_EMPTY$ + ")") + subexp("\\?(" + QUERY$ + ")") + subexp("\\#(" + FRAGMENT$ + ")"), "" + subexp(subexp("\\/\\/(" + subexp("(" + USERINFO$ + ")@") + "?(" + HOST$ + ")" + subexp("\\:(" + PORT$ + ")") + "?)") + "?(" + PATH_ABEMPTY$ + "|" + PATH_ABSOLUTE$ + "|" + PATH_NOSCHEME$ + "|" + PATH_EMPTY$ + ")") + subexp("\\?(" + QUERY$ + ")") + subexp("\\#(" + FRAGMENT$ + ")"), "" + SCHEME$ + subexp(subexp("\\/\\/(" + subexp("(" + USERINFO$ + ")@") + "?(" + HOST$ + ")" + subexp("\\:(" + PORT$ + ")") + "?)") + "?(" + PATH_ABEMPTY$ + "|" + PATH_ABSOLUTE$ + "|" + PATH_ROOTLESS$ + "|" + PATH_EMPTY$ + ")") + subexp("\\?(" + QUERY$ + ")"), "" + subexp("\\#(" + FRAGMENT$ + ")"), "" + subexp("(" + USERINFO$ + ")@") + HOST$ + subexp("\\:(" + PORT$ + ")"), {
				...
				IPV4ADDRESS: /* @__PURE__ */ RegExp("^(" + IPV4ADDRESS$ + ")$"),
				IPV6ADDRESS: /* @__PURE__ */ RegExp("^\\[?(" + IPV6ADDRESS$ + ")" + subexp(subexp("\\%25|\\%(?!" + HEXDIG$$$1 + "{2})") + "(" + ZONEID$ + ")") + "?\\]?$")
			};
		}
		...
		function error$1(type$1) {
			throw RangeError(errors[type$1]);
		}
		...
		var ucs2encode = function ucs2encode$1(array) {
			...
			for (var index$1 = basic > 0 ? basic + 1 : 0; index$1 < inputLength;) {
				for (var oldi = i, w = 1, k = base;; k += base) {
					index$1 >= inputLength && error$1("invalid-input");
					var digit = basicToDigit(input.charCodeAt(index$1++));
		...
		function _normalizeComponentEncoding(components, protocol) {
			...
			return components.scheme &&= String(components.scheme).replace(protocol.PCT_ENCODED, decodeUnreserved$1).toLowerCase().replace(protocol.NOT_SCHEME, ""), components.userinfo !== void 0 && (components.userinfo = String(components.userinfo).replace(protocol.PCT_ENCODED, decodeUnreserved$1).replace(protocol.NOT_USERINFO, pctEncChar).replace(protocol.PCT_ENCODED, toUpperCase)), components.host !== void 0 && (components.host = String(components.host).replace(protocol.PCT_ENCODED, decodeUnreserved$1).toLowerCase().replace(protocol.NOT_HOST, pctEncChar).replace(protocol.PCT_ENCODED, toUpperCase)), components.path !== void 0 && (components.path = String(components.path).replace(protocol.PCT_ENCODED, decodeUnreserved$1).replace(components.scheme ? protocol.NOT_PATH : protocol.NOT_PATH_NOSCHEME, pctEncChar).replace(protocol.PCT_ENCODED, toUpperCase)), components.query !== void 0 && (components.query = String(components.query).replace(protocol.PCT_ENCODED, decodeUnreserved$1).replace(protocol.NOT_QUERY, pctEncChar).replace(protocol.PCT_ENCODED, toUpperCase)), components.fragment !== void 0 && (components.fragment = String(components.fragment).replace(protocol.PCT_ENCODED, decodeUnreserved$1).replace(protocol.NOT_FRAGMENT, pctEncChar).replace(protocol.PCT_ENCODED, toUpperCase)), components;
		}
		...
		function _normalizeIPv6(host, protocol) {
			...
			if (address) {
				...
				var longestZeroFields = fields.reduce(function(acc, field, index$1) {
					if (!field || field === "0") {
						var lastLongest = acc[acc.length - 1];
						lastLongest && lastLongest.index + lastLongest.length === index$1 ? lastLongest.length++ : acc.push({
							index: index$1,
							length: 1
						});
					}
		...
		function resolveComponents(base$1, relative) {
			...
			return skipNormalization || (base$1 = parse(serialize(base$1, options), options), relative = parse(serialize(relative, options), options)), options ||= {}, !options.tolerant && relative.scheme ? (target.scheme = relative.scheme, target.userinfo = relative.userinfo, target.host = relative.host, target.port = relative.port, target.path = removeDotSegments(relative.path || ""), target.query = relative.query) : (relative.userinfo !== void 0 || relative.host !== void 0 || relative.port !== void 0 ? (target.userinfo = relative.userinfo, target.host = relative.host, target.port = relative.port, target.path = removeDotSegments(relative.path || ""), target.query = relative.query) : (relative.path ? (relative.path.charAt(0) === "/" ? target.path = removeDotSegments(relative.path) : ((base$1.userinfo !== void 0 || base$1.host !== void 0 || base$1.port !== void 0) && !base$1.path ? target.path = "/" + relative.path : base$1.path ? target.path = base$1.path.slice(0, base$1.path.lastIndexOf("/") + 1) + relative.path : target.path = relative.path, target.path = removeDotSegments(target.path)), target.query = relative.query) : (target.path = base$1.path, relative.query === void 0 ? target.query = base$1.query : target.query = relative.query), target.userinfo = base$1.userinfo, target.host = base$1.host, target.port = base$1.port), target.scheme = base$1.scheme), target.fragment = relative.fragment, target;
		}
	...
	function resolveSchema(root, ref) {
		...
		if (Object.keys(root.schema).length === 0 || refPath !== baseId) {
			var id$1 = normalizeId(refPath), refVal = this._refs[id$1];
			...
			else if (refVal = this._schemas[id$1], refVal instanceof SchemaObject) {
				if (refVal.validate || this._compile(refVal), id$1 == normalizeId(ref)) return {
					schema: refVal,
					root,
					baseId
				};
	...
	function resolveRecursive(root, ref, parsedRef) {
		...
		if (res) {
			...
			root = res.root;
			var id$1 = this._getId(schema);
			return id$1 && (baseId = resolveUrl(baseId, id$1)), getJsonPointer.call(this, parsedRef, baseId, schema, root);
		}
	}
		...
		if (parsedRef.fragment = parsedRef.fragment || "", parsedRef.fragment.slice(0, 1) == "/") {
			for (var parts = parsedRef.fragment.split("/"), i = 1; i < parts.length; i++) {
				...
				if (part) {
					...
					var id$1;
					if (!PREVENT_SCOPE_CHANGE[part] && (id$1 = this._getId(schema), id$1 && (baseId = resolveUrl(baseId, id$1)), schema.$ref)) {
	...
	function getFullPath(id$1, normalize) {
		return normalize !== !1 && (id$1 = normalizeId(id$1)), _getFullPath(URI.parse(id$1));
	}
	function _getFullPath(p) {
		return URI.serialize(p).split("#")[0] + "#";
	}
	var TRAILING_SLASH_HASH = /#\/?$/;
	function normalizeId(id$1) {
		return id$1 ? id$1.replace(TRAILING_SLASH_HASH, "") : "";
	}
	function resolveUrl(baseId, id$1) {
		return id$1 = normalizeId(id$1), URI.resolve(baseId, id$1);
	}
	function resolveIds(schema) {
		var schemaId = normalizeId(this._getId(schema)), baseIds = { "": schemaId }, fullPaths = { "": getFullPath(schemaId, !1) }, localRefs = {}, self$1 = this;
		return traverse(schema, { allKeys: !0 }, function(sch, jsonPtr, rootSchema, parentJsonPtr, parentKeyword, parentSchema, keyIndex) {
			if (jsonPtr !== "") {
				var id$1 = self$1._getId(sch), baseId = baseIds[parentJsonPtr], fullPath = fullPaths[parentJsonPtr] + "/" + parentKeyword;
				if (keyIndex !== void 0 && (fullPath += "/" + (typeof keyIndex == "number" ? keyIndex : util.escapeFragment(keyIndex))), typeof id$1 == "string") {
					id$1 = baseId = normalizeId(baseId ? URI.resolve(baseId, id$1) : id$1);
					var refVal = self$1._refs[id$1];
					if (typeof refVal == "string" && (refVal = self$1._refs[refVal]), refVal && refVal.schema) {
						if (!equal(sch, refVal.schema)) throw Error("id \"" + id$1 + "\" resolves to more than one schema");
					} else if (id$1 != normalizeId(fullPath)) if (id$1[0] == "#") {
						if (localRefs[id$1] && !equal(sch, localRefs[id$1])) throw Error("id \"" + id$1 + "\" resolves to more than one schema");
						localRefs[id$1] = sch;
					} else self$1._refs[id$1] = fullPath;
				}
	...
	module.exports = function generate_validate(it, $keyword, $ruleType) {
		...
		if (it.isTop && (out += " var validate = ", $async && (it.async = !0, out += "async "), out += "function(data, dataPath, parentData, parentDataProperty, rootData) { 'use strict'; ", $id && (it.opts.sourceCode || it.opts.processCode) && (out += " " + ("/*# sourceURL=" + $id + " */") + " ")), typeof it.schema == "boolean" || !($refKeywords || it.schema.$ref)) {
			var $keyword = "false schema", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $errorKeyword, $data = "data" + ($dataLvl || ""), $valid = "valid" + $lvl;
			if (it.schema === !1) {
				...
				for (var $rulesGroup, i2 = -1, l2 = arr2.length - 1; i2 < l2;) if ($rulesGroup = arr2[i2 += 1], $shouldUseGroup($rulesGroup)) {
					if ($rulesGroup.type && (out += " if (" + it.util.checkDataType($rulesGroup.type, $data, it.opts.strictNumbers) + ") { "), it.opts.useDefaults) {
						if ($rulesGroup.type == "object" && it.schema.properties) {
							var $schema$1 = it.schema.properties, arr3 = Object.keys($schema$1);
							if (arr3) for (var $propertyKey, i3 = -1, l3 = arr3.length - 1; i3 < l3;) {
								$propertyKey = arr3[i3 += 1];
								var $sch = $schema$1[$propertyKey];
		...
		function $shouldUseGroup($rulesGroup$1) {
			for (var rules = $rulesGroup$1.rules, i = 0; i < rules.length; i++) if ($shouldUseRule(rules[i])) return !0;
		}
		...
		function $ruleImplementsSomeKeyword($rule$1) {
			for (var impl = $rule$1.implements, i = 0; i < impl.length; i++) if (it.schema[impl[i]] !== void 0) return !0;
		}
		return out;
	};
	...
	function compile(schema, root, localRefs, baseId) {
		var self$1 = this, opts = this._opts, refVal = [void 0], refs = {}, patterns = [], patternsHash = {}, defaults = [], defaultsHash = {}, customRules = [];
		root ||= {
			schema,
			refVal,
			refs
		...
		function localCompile(_schema, _root, localRefs$1, baseId$1) {
			...
			if (_root.schema != root.schema) return compile.call(self$1, _schema, _root, localRefs$1, baseId$1);
			...
			var validate;
			try {
				validate = Function("self", "RULES", "formats", "root", "refVal", "defaults", "customRules", "equal", "ucs2length", "ValidationError", sourceCode)(self$1, RULES, formats, root, refVal, defaults, customRules, equal, ucs2length, ValidationError), refVal[0] = validate;
			} catch (e) {
				throw self$1.logger.error("Error compiling schema, function code:", sourceCode), e;
			}
		...
		function resolveRef(baseId$1, ref, isRoot) {
			...
			var v$1 = resolve.call(self$1, localCompile, root, ref);
			if (v$1 === void 0) {
				var localSchema = localRefs && localRefs[ref];
				localSchema && (v$1 = resolve.inlineRef(localSchema, opts.inlineRefs) ? localSchema : compile.call(self$1, localSchema, root, localRefs, baseId$1));
			}
		...
		function usePattern(regexStr) {
			var index$1 = patternsHash[regexStr];
			return index$1 === void 0 && (index$1 = patternsHash[regexStr] = patterns.length, patterns[index$1] = regexStr), "pattern" + index$1;
		}
		function useDefault(value) {
			switch (typeof value) {
				...
				case "object":
					...
					var valueStr = stableStringify(value), index$1 = defaultsHash[valueStr];
					return index$1 === void 0 && (index$1 = defaultsHash[valueStr] = defaults.length, defaults[index$1] = value), "default" + index$1;
			}
		}
		function useCustomRule(rule, schema$1, parentSchema, it) {
			if (self$1._opts.validateSchema !== !1) {
				var deps = rule.definition.dependencies;
				if (deps && !deps.every(function(keyword) {
					return Object.prototype.hasOwnProperty.call(parentSchema, keyword);
				})) throw Error("parent schema must have all required keywords: " + deps.join(","));
				var validateSchema = rule.definition.validateSchema;
				if (validateSchema && !validateSchema(schema$1)) {
					var message = "keyword schema is invalid: " + self$1.errorsText(validateSchema.errors);
					if (self$1._opts.validateSchema == "log") self$1.logger.error(message);
					else throw Error(message);
				}
			}
			var compile = rule.definition.compile, inline = rule.definition.inline, macro = rule.definition.macro, validate;
			if (compile) validate = compile.call(self$1, schema$1, parentSchema, it);
			else if (macro) validate = macro.call(self$1, schema$1, parentSchema, it), opts.validateSchema !== !1 && self$1.validateSchema(validate, !0);
			else if (inline) validate = inline.call(self$1, it, rule.keyword, schema$1, parentSchema);
			...
			return customRules[index$1] = validate, {
	...
	function checkCompiling(schema, root, baseId) {
		var index$1 = compIndex.call(this, schema, root, baseId);
		return index$1 >= 0 ? {
			index: index$1,
			compiling: !0
		} : (index$1 = this._compilations.length, this._compilations[index$1] = {
			schema,
			root,
			baseId
		}, {
	...
	module.exports = function generate_ref(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $data = "data" + ($dataLvl || ""), $valid = "valid" + $lvl, $async, $refCode;
		if ($schema$1 == "#" || $schema$1 == "#/") it.isRoot ? ($async = it.async, $refCode = "validate") : ($async = it.root.schema.$async === !0, $refCode = "root.refVal[0]");
		else {
			var $refVal = it.resolveRef(it.baseId, $schema$1, it.isRoot);
			if ($refVal === void 0) {
				var $message = it.MissingRefError.message(it.baseId, $schema$1);
				if (it.opts.missingRefs == "fail") {
					it.logger.error($message);
					var $$outStack = $$outStack || [];
					$$outStack.push(out), out = "", it.createErrors === !1 ? out += " {} " : (out += " { keyword: '$ref' , dataPath: (dataPath || '') + " + it.errorPath + " , schemaPath: " + it.util.toQuotedString($errSchemaPath) + " , params: { ref: '" + it.util.escapeQuotes($schema$1) + "' } ", it.opts.messages !== !1 && (out += " , message: 'can\\'t resolve reference " + it.util.escapeQuotes($schema$1) + "' "), it.opts.verbose && (out += " , schema: " + it.util.toQuotedString($schema$1) + " , parentSchema: validate.schema" + it.schemaPath + " , data: " + $data + " "), out += " } ");
					var __err = out;
				...
				else throw new it.MissingRefError(it.baseId, $schema$1, $message);
			} else if ($refVal.inline) {
				var $it = it.util.copy(it);
				$it.level++;
				var $nextValid = "valid" + $it.level;
				$it.schema = $refVal.schema, $it.schemaPath = "", $it.errSchemaPath = $schema$1;
	...
	module.exports = function generate_allOf(it, $keyword, $ruleType) {
		var out = " ", $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $it = it.util.copy(it), $closingBraces = "";
		$it.level++;
		var $nextValid = "valid" + $it.level, $currentBaseId = $it.baseId, $allSchemasEmpty = !0, arr1 = $schema$1;
	...
	module.exports = function generate_anyOf(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $data = "data" + ($dataLvl || ""), $valid = "valid" + $lvl, $errs = "errs__" + $lvl, $it = it.util.copy(it), $closingBraces = "";
		$it.level++;
		var $nextValid = "valid" + $it.level;
		if ($schema$1.every(function($sch$1) {
			return it.opts.strictKeywords ? typeof $sch$1 == "object" && Object.keys($sch$1).length > 0 || $sch$1 === !1 : it.util.schemaHasRules($sch$1, it.RULES.all);
		})) {
	...
	module.exports = function generate_comment(it, $keyword, $ruleType) {
		var out = " ", $schema$1 = it.schema[$keyword], $errSchemaPath = it.errSchemaPath + "/" + $keyword;
		it.opts.allErrors;
		var $comment = it.util.toQuotedString($schema$1);
	...
	module.exports = function generate_const(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $data = "data" + ($dataLvl || ""), $valid = "valid" + $lvl, $isData = it.opts.$data && $schema$1 && $schema$1.$data;
		$isData && (out += " var schema" + $lvl + " = " + it.util.getData($schema$1.$data, $dataLvl, it.dataPathArr) + "; ", "" + $lvl), $isData || (out += " var schema" + $lvl + " = validate.schema" + $schemaPath + ";"), out += "var " + $valid + " = equal(" + $data + ", schema" + $lvl + "); if (!" + $valid + ") {   ";
		var $$outStack = $$outStack || [];
	...
	module.exports = function generate_contains(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $data = "data" + ($dataLvl || ""), $valid = "valid" + $lvl, $errs = "errs__" + $lvl, $it = it.util.copy(it), $closingBraces = "";
		$it.level++;
		var $nextValid = "valid" + $it.level, $idx = "i" + $lvl, $dataNxt = $it.dataLevel = it.dataLevel + 1, $nextData = "data" + $dataNxt, $currentBaseId = it.baseId, $nonEmptySchema = it.opts.strictKeywords ? typeof $schema$1 == "object" && Object.keys($schema$1).length > 0 || $schema$1 === !1 : it.util.schemaHasRules($schema$1, it.RULES.all);
		if (out += "var " + $errs + " = errors;var " + $valid + ";", $nonEmptySchema) {
			var $wasComposite = it.compositeRule;
			it.compositeRule = $it.compositeRule = !0, $it.schema = $schema$1, $it.schemaPath = $schemaPath, $it.errSchemaPath = $errSchemaPath, out += " var " + $nextValid + " = false; for (var " + $idx + " = 0; " + $idx + " < " + $data + ".length; " + $idx + "++) { ", $it.errorPath = it.util.getPathExpr(it.errorPath, $idx, it.opts.jsonPointers, !0);
	...
	module.exports = function generate_dependencies(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $data = "data" + ($dataLvl || ""), $errs = "errs__" + $lvl, $it = it.util.copy(it), $closingBraces = "";
		$it.level++;
		var $nextValid = "valid" + $it.level, $schemaDeps = {}, $propertyDeps = {}, $ownProperties = it.opts.ownProperties;
		for ($property in $schema$1) if ($property != "__proto__") {
			var $sch = $schema$1[$property], $deps = Array.isArray($sch) ? $propertyDeps : $schemaDeps;
			$deps[$property] = $sch;
		}
		out += "var " + $errs + " = errors;";
	...
	module.exports = function generate_enum(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $data = "data" + ($dataLvl || ""), $valid = "valid" + $lvl, $isData = it.opts.$data && $schema$1 && $schema$1.$data;
		$isData && (out += " var schema" + $lvl + " = " + it.util.getData($schema$1.$data, $dataLvl, it.dataPathArr) + "; ", "" + $lvl);
	...
	module.exports = function generate_format(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $data = "data" + ($dataLvl || "");
		if (it.opts.format === !1) return $breakOnError && (out += " if (true) { "), out;
		var $isData = it.opts.$data && $schema$1 && $schema$1.$data, $schemaValue;
		$isData ? (out += " var schema" + $lvl + " = " + it.util.getData($schema$1.$data, $dataLvl, it.dataPathArr) + "; ", $schemaValue = "schema" + $lvl) : $schemaValue = $schema$1;
		...
		if ($isData) {
			...
			var $format = it.formats[$schema$1];
			if (!$format) {
				if ($unknownFormats == "ignore") return it.logger.warn("unknown format \"" + $schema$1 + "\" ignored in schema at path \"" + it.errSchemaPath + "\""), $breakOnError && (out += " if (true) { "), out;
				if ($allowUnknown && $unknownFormats.indexOf($schema$1) >= 0) return $breakOnError && (out += " if (true) { "), out;
				throw Error("unknown format \"" + $schema$1 + "\" is used in schema at path \"" + it.errSchemaPath + "\"");
			}
			...
			if ($async) {
				...
				var $formatRef = "formats" + it.util.getProperty($schema$1) + ".validate";
				out += " if (!(await " + $formatRef + "(" + $data + "))) { ";
			} else {
				out += " if (! ";
				var $formatRef = "formats" + it.util.getProperty($schema$1);
				$isObject && ($formatRef += ".validate"), typeof $format == "function" ? out += " " + $formatRef + "(" + $data + ") " : out += " " + $formatRef + ".test(" + $data + ") ", out += ") { ";
			}
		}
		var $$outStack = $$outStack || [];
		$$outStack.push(out), out = "", it.createErrors === !1 ? out += " {} " : (out += " { keyword: 'format' , dataPath: (dataPath || '') + " + it.errorPath + " , schemaPath: " + it.util.toQuotedString($errSchemaPath) + " , params: { format:  ", $isData ? out += "" + $schemaValue : out += "" + it.util.toQuotedString($schema$1), out += "  } ", it.opts.messages !== !1 && (out += " , message: 'should match format \"", $isData ? out += "' + " + $schemaValue + " + '" : out += "" + it.util.escapeQuotes($schema$1), out += "\"' "), it.opts.verbose && (out += " , schema:  ", $isData ? out += "validate.schema" + $schemaPath : out += "" + it.util.toQuotedString($schema$1), out += "         , parentSchema: validate.schema" + it.schemaPath + " , data: " + $data + " "), out += " } ");
		var __err = out;
		return out = $$outStack.pop(), !it.compositeRule && $breakOnError ? it.async ? out += " throw new ValidationError([" + __err + "]); " : out += " validate.errors = [" + __err + "]; return false; " : out += " var err = " + __err + ";  if (vErrors === null) vErrors = [err]; else vErrors.push(err); errors++; ", out += " } ", $breakOnError && (out += " else { "), out;
	};
})), require_if = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = function generate_if(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $data = "data" + ($dataLvl || ""), $valid = "valid" + $lvl, $errs = "errs__" + $lvl, $it = it.util.copy(it);
		$it.level++;
		...
		if ($thenPresent || $elsePresent) {
			var $ifClause;
			$it.createErrors = !1, $it.schema = $schema$1, $it.schemaPath = $schemaPath, $it.errSchemaPath = $errSchemaPath, out += " var " + $errs + " = errors; var " + $valid + " = true;  ";
			var $wasComposite = it.compositeRule;
	...
	module.exports = function generate_items(it, $keyword, $ruleType) {
		...
		if (out += "var " + $errs + " = errors;var " + $valid + ";", Array.isArray($schema$1)) {
			var $additionalItems = it.schema.additionalItems;
			if ($additionalItems === !1) {
				out += " " + $valid + " = " + $data + ".length <= " + $schema$1.length + "; ";
				var $currErrSchemaPath = $errSchemaPath;
				$errSchemaPath = it.errSchemaPath + "/additionalItems", out += "  if (!" + $valid + ") {   ";
				var $$outStack = $$outStack || [];
				$$outStack.push(out), out = "", it.createErrors === !1 ? out += " {} " : (out += " { keyword: 'additionalItems' , dataPath: (dataPath || '') + " + it.errorPath + " , schemaPath: " + it.util.toQuotedString($errSchemaPath) + " , params: { limit: " + $schema$1.length + " } ", it.opts.messages !== !1 && (out += " , message: 'should NOT have more than " + $schema$1.length + " items' "), it.opts.verbose && (out += " , schema: false , parentSchema: validate.schema" + it.schemaPath + " , data: " + $data + " "), out += " } ");
				var __err = out;
			...
			if (typeof $additionalItems == "object" && (it.opts.strictKeywords ? typeof $additionalItems == "object" && Object.keys($additionalItems).length > 0 || $additionalItems === !1 : it.util.schemaHasRules($additionalItems, it.RULES.all))) {
				$it.schema = $additionalItems, $it.schemaPath = it.schemaPath + ".additionalItems", $it.errSchemaPath = it.errSchemaPath + "/additionalItems", out += " " + $nextValid + " = true; if (" + $data + ".length > " + $schema$1.length + ") {  for (var " + $idx + " = " + $schema$1.length + "; " + $idx + " < " + $data + ".length; " + $idx + "++) { ", $it.errorPath = it.util.getPathExpr(it.errorPath, $idx, it.opts.jsonPointers, !0);
				var $passData = $data + "[" + $idx + "]";
				$it.dataPathArr[$dataNxt] = $idx;
				var $code = it.validate($it);
		...
		} else if (it.opts.strictKeywords ? typeof $schema$1 == "object" && Object.keys($schema$1).length > 0 || $schema$1 === !1 : it.util.schemaHasRules($schema$1, it.RULES.all)) {
			$it.schema = $schema$1, $it.schemaPath = $schemaPath, $it.errSchemaPath = $errSchemaPath, out += "  for (var " + $idx + " = 0; " + $idx + " < " + $data + ".length; " + $idx + "++) { ", $it.errorPath = it.util.getPathExpr(it.errorPath, $idx, it.opts.jsonPointers, !0);
			var $passData = $data + "[" + $idx + "]";
			$it.dataPathArr[$dataNxt] = $idx;
			var $code = it.validate($it);
	...
	module.exports = function generate__limit(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $errorKeyword, $data = "data" + ($dataLvl || ""), $isData = it.opts.$data && $schema$1 && $schema$1.$data, $schemaValue;
		$isData ? (out += " var schema" + $lvl + " = " + it.util.getData($schema$1.$data, $dataLvl, it.dataPathArr) + "; ", $schemaValue = "schema" + $lvl) : $schemaValue = $schema$1;
		var $isMax = $keyword == "maximum", $exclusiveKeyword = $isMax ? "exclusiveMaximum" : "exclusiveMinimum", $schemaExcl = it.schema[$exclusiveKeyword], $isDataExcl = it.opts.$data && $schemaExcl && $schemaExcl.$data, $op = $isMax ? "<" : ">", $notOp = $isMax ? ">" : "<", $errorKeyword = void 0;
		if (!($isData || typeof $schema$1 == "number" || $schema$1 === void 0)) throw Error($keyword + " must be number");
		...
		if ($isDataExcl) {
			...
			if ($exclIsNumber && $isData) {
				...
				$exclIsNumber && $schema$1 === void 0 ? ($exclusive = !0, $errorKeyword = $exclusiveKeyword, $errSchemaPath = it.errSchemaPath + "/" + $exclusiveKeyword, $schemaValue = $schemaExcl, $notOp += "=") : ($exclIsNumber && ($schemaValue = Math[$isMax ? "min" : "max"]($schemaExcl, $schema$1)), $schemaExcl === ($exclIsNumber ? $schemaValue : !0) ? ($exclusive = !0, $errorKeyword = $exclusiveKeyword, $errSchemaPath = it.errSchemaPath + "/" + $exclusiveKeyword, $notOp += "=") : ($exclusive = !1, $opStr += "="));
				var $opExpr = "'" + $opStr + "'";
		...
		var $$outStack = $$outStack || [];
		$$outStack.push(out), out = "", it.createErrors === !1 ? out += " {} " : (out += " { keyword: '" + ($errorKeyword || "_limit") + "' , dataPath: (dataPath || '') + " + it.errorPath + " , schemaPath: " + it.util.toQuotedString($errSchemaPath) + " , params: { comparison: " + $opExpr + ", limit: " + $schemaValue + ", exclusive: " + $exclusive + " } ", it.opts.messages !== !1 && (out += " , message: 'should be " + $opStr + " ", $isData ? out += "' + " + $schemaValue : out += "" + $schemaValue + "'"), it.opts.verbose && (out += " , schema:  ", $isData ? out += "validate.schema" + $schemaPath : out += "" + $schema$1, out += "         , parentSchema: validate.schema" + it.schemaPath + " , data: " + $data + " "), out += " } ");
		var __err = out;
		return out = $$outStack.pop(), !it.compositeRule && $breakOnError ? it.async ? out += " throw new ValidationError([" + __err + "]); " : out += " validate.errors = [" + __err + "]; return false; " : out += " var err = " + __err + ";  if (vErrors === null) vErrors = [err]; else vErrors.push(err); errors++; ", out += " } ", $breakOnError && (out += " else { "), out;
	};
	...
	module.exports = function generate__limitItems(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $errorKeyword, $data = "data" + ($dataLvl || ""), $isData = it.opts.$data && $schema$1 && $schema$1.$data, $schemaValue;
		if ($isData ? (out += " var schema" + $lvl + " = " + it.util.getData($schema$1.$data, $dataLvl, it.dataPathArr) + "; ", $schemaValue = "schema" + $lvl) : $schemaValue = $schema$1, !($isData || typeof $schema$1 == "number")) throw Error($keyword + " must be number");
	...
	module.exports = function generate__limitProperties(it, $keyword, $ruleType) {
		...
		var $errorKeyword = $keyword, $$outStack = $$outStack || [];
		$$outStack.push(out), out = "", it.createErrors === !1 ? out += " {} " : (out += " { keyword: '" + ($errorKeyword || "_limitProperties") + "' , dataPath: (dataPath || '') + " + it.errorPath + " , schemaPath: " + it.util.toQuotedString($errSchemaPath) + " , params: { limit: " + $schemaValue + " } ", it.opts.messages !== !1 && (out += " , message: 'should NOT have ", $keyword == "maxProperties" ? out += "more" : out += "fewer", out += " than ", $isData ? out += "' + " + $schemaValue + " + '" : out += "" + $schema$1, out += " properties' "), it.opts.verbose && (out += " , schema:  ", $isData ? out += "validate.schema" + $schemaPath : out += "" + $schema$1, out += "         , parentSchema: validate.schema" + it.schemaPath + " , data: " + $data + " "), out += " } ");
		var __err = out;
		return out = $$outStack.pop(), !it.compositeRule && $breakOnError ? it.async ? out += " throw new ValidationError([" + __err + "]); " : out += " validate.errors = [" + __err + "]; return false; " : out += " var err = " + __err + ";  if (vErrors === null) vErrors = [err]; else vErrors.push(err); errors++; ", out += "} ", $breakOnError && (out += " else { "), out;
	};
})), require_multipleOf = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = function generate_multipleOf(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $data = "data" + ($dataLvl || ""), $isData = it.opts.$data && $schema$1 && $schema$1.$data, $schemaValue;
		if ($isData ? (out += " var schema" + $lvl + " = " + it.util.getData($schema$1.$data, $dataLvl, it.dataPathArr) + "; ", $schemaValue = "schema" + $lvl) : $schemaValue = $schema$1, !($isData || typeof $schema$1 == "number")) throw Error($keyword + " must be number");
		...
		var $$outStack = $$outStack || [];
		$$outStack.push(out), out = "", it.createErrors === !1 ? out += " {} " : (out += " { keyword: 'multipleOf' , dataPath: (dataPath || '') + " + it.errorPath + " , schemaPath: " + it.util.toQuotedString($errSchemaPath) + " , params: { multipleOf: " + $schemaValue + " } ", it.opts.messages !== !1 && (out += " , message: 'should be multiple of ", $isData ? out += "' + " + $schemaValue : out += "" + $schemaValue + "'"), it.opts.verbose && (out += " , schema:  ", $isData ? out += "validate.schema" + $schemaPath : out += "" + $schema$1, out += "         , parentSchema: validate.schema" + it.schemaPath + " , data: " + $data + " "), out += " } ");
		var __err = out;
		return out = $$outStack.pop(), !it.compositeRule && $breakOnError ? it.async ? out += " throw new ValidationError([" + __err + "]); " : out += " validate.errors = [" + __err + "]; return false; " : out += " var err = " + __err + ";  if (vErrors === null) vErrors = [err]; else vErrors.push(err); errors++; ", out += "} ", $breakOnError && (out += " else { "), out;
	};
})), require_not = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = function generate_not(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $data = "data" + ($dataLvl || ""), $errs = "errs__" + $lvl, $it = it.util.copy(it);
		$it.level++;
		var $nextValid = "valid" + $it.level;
		if (it.opts.strictKeywords ? typeof $schema$1 == "object" && Object.keys($schema$1).length > 0 || $schema$1 === !1 : it.util.schemaHasRules($schema$1, it.RULES.all)) {
			$it.schema = $schema$1, $it.schemaPath = $schemaPath, $it.errSchemaPath = $errSchemaPath, out += " var " + $errs + " = errors;  ";
			var $wasComposite = it.compositeRule;
	...
	module.exports = function generate_pattern(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $data = "data" + ($dataLvl || ""), $isData = it.opts.$data && $schema$1 && $schema$1.$data, $schemaValue;
		$isData ? (out += " var schema" + $lvl + " = " + it.util.getData($schema$1.$data, $dataLvl, it.dataPathArr) + "; ", $schemaValue = "schema" + $lvl) : $schemaValue = $schema$1;
		var $regexp = $isData ? "(new RegExp(" + $schemaValue + "))" : it.usePattern($schema$1);
		out += "if ( ", $isData && (out += " (" + $schemaValue + " !== undefined && typeof " + $schemaValue + " != 'string') || "), out += " !" + $regexp + ".test(" + $data + ") ) {   ";
		var $$outStack = $$outStack || [];
		$$outStack.push(out), out = "", it.createErrors === !1 ? out += " {} " : (out += " { keyword: 'pattern' , dataPath: (dataPath || '') + " + it.errorPath + " , schemaPath: " + it.util.toQuotedString($errSchemaPath) + " , params: { pattern:  ", $isData ? out += "" + $schemaValue : out += "" + it.util.toQuotedString($schema$1), out += "  } ", it.opts.messages !== !1 && (out += " , message: 'should match pattern \"", $isData ? out += "' + " + $schemaValue + " + '" : out += "" + it.util.escapeQuotes($schema$1), out += "\"' "), it.opts.verbose && (out += " , schema:  ", $isData ? out += "validate.schema" + $schemaPath : out += "" + it.util.toQuotedString($schema$1), out += "         , parentSchema: validate.schema" + it.schemaPath + " , data: " + $data + " "), out += " } ");
		var __err = out;
		return out = $$outStack.pop(), !it.compositeRule && $breakOnError ? it.async ? out += " throw new ValidationError([" + __err + "]); " : out += " validate.errors = [" + __err + "]; return false; " : out += " var err = " + __err + ";  if (vErrors === null) vErrors = [err]; else vErrors.push(err); errors++; ", out += "} ", $breakOnError && (out += " else { "), out;
	};
	...
	module.exports = function generate_propertyNames(it, $keyword, $ruleType) {
		...
		$it.level++;
		var $nextValid = "valid" + $it.level;
		if (out += "var " + $errs + " = errors;", it.opts.strictKeywords ? typeof $schema$1 == "object" && Object.keys($schema$1).length > 0 || $schema$1 === !1 : it.util.schemaHasRules($schema$1, it.RULES.all)) {
			$it.schema = $schema$1, $it.schemaPath = $schemaPath, $it.errSchemaPath = $errSchemaPath;
	...
	module.exports = function generate_required(it, $keyword, $ruleType) {
		...
		if (!$isData) if ($schema$1.length < it.opts.loopRequired && it.schema.properties && Object.keys(it.schema.properties).length) {
			var $required = [], arr1 = $schema$1;
	...
	module.exports = function generate_uniqueItems(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $data = "data" + ($dataLvl || ""), $valid = "valid" + $lvl, $isData = it.opts.$data && $schema$1 && $schema$1.$data, $schemaValue;
		if ($isData ? (out += " var schema" + $lvl + " = " + it.util.getData($schema$1.$data, $dataLvl, it.dataPathArr) + "; ", $schemaValue = "schema" + $lvl) : $schemaValue = $schema$1, ($schema$1 || $isData) && it.opts.uniqueItems !== !1) {
			...
			var $$outStack = $$outStack || [];
			$$outStack.push(out), out = "", it.createErrors === !1 ? out += " {} " : (out += " { keyword: 'uniqueItems' , dataPath: (dataPath || '') + " + it.errorPath + " , schemaPath: " + it.util.toQuotedString($errSchemaPath) + " , params: { i: i, j: j } ", it.opts.messages !== !1 && (out += " , message: 'should NOT have duplicate items (items ## ' + j + ' and ' + i + ' are identical)' "), it.opts.verbose && (out += " , schema:  ", $isData ? out += "validate.schema" + $schemaPath : out += "" + $schema$1, out += "         , parentSchema: validate.schema" + it.schemaPath + " , data: " + $data + " "), out += " } ");
			var __err = out;
	...
	function compileAsync(schema, meta, callback) {
		...
		var p = loadMetaSchemaOf(schema).then(function() {
			var schemaObj = self$1._addSchema(schema, void 0, meta);
		...
		function loadMetaSchemaOf(sch) {
			var $schema$1 = sch.$schema;
			return $schema$1 && !self$1.getSchema($schema$1) ? compileAsync.call(self$1, { $ref: $schema$1 }, !0) : Promise.resolve();
		}
		function _compileAsync(schemaObj) {
			try {
				return self$1._compile(schemaObj);
			} catch (e) {
			...
			function loadMissingSchema(e) {
				...
				var schemaPromise = self$1._loadingSchemas[ref];
				return schemaPromise || (schemaPromise = self$1._loadingSchemas[ref] = self$1._opts.loadSchema(ref), schemaPromise.then(removePromise, removePromise)), schemaPromise.then(function(sch) {
					if (!added(ref)) return loadMetaSchemaOf(sch).then(function() {
						added(ref) || self$1.addSchema(sch, ref, void 0, meta);
					});
				}).then(function() {
					return _compileAsync(schemaObj);
				});
				function removePromise() {
					delete self$1._loadingSchemas[ref];
				}
				function added(ref$1) {
					return self$1._refs[ref$1] || self$1._schemas[ref$1];
				}
			}
		}
	}
	...
	module.exports = function generate_custom(it, $keyword, $ruleType) {
		var out = " ", $lvl = it.level, $dataLvl = it.dataLevel, $schema$1 = it.schema[$keyword], $schemaPath = it.schemaPath + it.util.getProperty($keyword), $errSchemaPath = it.errSchemaPath + "/" + $keyword, $breakOnError = !it.opts.allErrors, $errorKeyword, $data = "data" + ($dataLvl || ""), $valid = "valid" + $lvl, $errs = "errs__" + $lvl, $isData = it.opts.$data && $schema$1 && $schema$1.$data, $schemaValue;
		$isData ? (out += " var schema" + $lvl + " = " + it.util.getData($schema$1.$data, $dataLvl, it.dataPathArr) + "; ", $schemaValue = "schema" + $lvl) : $schemaValue = $schema$1;
		...
		if ($isData && $rDef.$data) {
			...
			if ($ruleValidate = it.useCustomRule($rule, $schema$1, it.schema, it), !$ruleValidate) return;
	...
	function addSchema(schema, key, _skipValidation, _meta) {
		...
		var id$1 = this._getId(schema);
		if (id$1 !== void 0 && typeof id$1 != "string") throw Error("schema id must be string");
		return key = resolve.normalizeId(key || id$1), checkUnique(this, key), this._schemas[key] = this._addSchema(schema, _skipValidation, _meta, !0), this;
	}
	function addMetaSchema(schema, key, skipValidation) {
		return this.addSchema(schema, key, skipValidation, !0), this;
	}
	function validateSchema(schema, throwOrLogError) {
		var $schema$1 = schema.$schema;
		if ($schema$1 !== void 0 && typeof $schema$1 != "string") throw Error("$schema must be a string");
		if ($schema$1 = $schema$1 || this._opts.defaultMeta || defaultMeta(this), !$schema$1) return this.logger.warn("meta-schema not available"), this.errors = null, !0;
		var valid = this.validate($schema$1, schema);
	...
	function defaultMeta(self$1) {
		var meta = self$1._opts.meta;
		return self$1._opts.defaultMeta = typeof meta == "object" ? self$1._getId(meta) || meta : self$1.getSchema(META_SCHEMA_ID) ? META_SCHEMA_ID : void 0, self$1._opts.defaultMeta;
	}
	...
	function _getSchemaFragment(self$1, ref) {
		var res = resolve.schema.call(self$1, { schema: {} }, ref);
		if (res) {
			var schema = res.schema, root = res.root, baseId = res.baseId, v = compileSchema.call(self$1, schema, root, void 0, baseId);
			return self$1._fragments[ref] = new SchemaObject({
				ref,
				fragment: !0,
				schema,
				root,
	...
	function _getSchemaObj(self$1, keyRef) {
		return keyRef = resolve.normalizeId(keyRef), self$1._schemas[keyRef] || self$1._refs[keyRef] || self$1._fragments[keyRef];
	}
	function removeSchema(schemaKeyRef) {
		...
		switch (typeof schemaKeyRef) {
			...
			case "object":
				...
				var id$1 = this._getId(schemaKeyRef);
				id$1 && (id$1 = resolve.normalizeId(id$1), delete this._schemas[id$1], delete this._refs[id$1]);
		}
		return this;
	}
	function _removeAllSchemas(self$1, schemas, regex) {
		for (var keyRef in schemas) {
			var schemaObj = schemas[keyRef];
			!schemaObj.meta && (!regex || regex.test(keyRef)) && (self$1._cache.del(schemaObj.cacheKey), delete schemas[keyRef]);
		}
	}
	function _addSchema(schema, skipValidation, meta, shouldAddSchema) {
		...
		var id$1 = resolve.normalizeId(this._getId(schema));
		id$1 && shouldAddSchema && checkUnique(this, id$1);
		var willValidate = this._opts.validateSchema !== !1 && !skipValidation, recursiveMeta;
		willValidate && !(recursiveMeta = id$1 && id$1 == resolve.normalizeId(schema.$schema)) && this.validateSchema(schema, !0);
		...
		return id$1[0] != "#" && shouldAddSchema && (this._refs[id$1] = schemaObj), this._cache.put(cacheKey, schemaObj), willValidate && recursiveMeta && this.validateSchema(schema, !0), schemaObj;
	}
	...
	function addDefaultMetaSchema(self$1) {
		var $dataSchema;
		if (self$1._opts.$data && ($dataSchema = require_data(), self$1.addMetaSchema($dataSchema, $dataSchema.$id, !0)), self$1._opts.meta !== !1) {
			var metaSchema = require_json_schema_draft_07();
			self$1._opts.$data && (metaSchema = $dataMetaSchema(metaSchema, META_SUPPORT_DATA)), self$1.addMetaSchema(metaSchema, META_SCHEMA_ID, !0), self$1._refs["http://json-schema.org/schema"] = META_SCHEMA_ID;
		}
	}
	function addInitialSchemas(self$1) {
		var optsSchemas = self$1._opts.schemas;
		if (optsSchemas) if (Array.isArray(optsSchemas)) self$1.addSchema(optsSchemas);
		else for (var key in optsSchemas) self$1.addSchema(optsSchemas[key], key);
	}
	function addInitialFormats(self$1) {
		for (var name in self$1._opts.formats) {
			var format = self$1._opts.formats[name];
			self$1.addFormat(name, format);
		}
	}
	function addInitialKeywords(self$1) {
		for (var name in self$1._opts.keywords) {
			var keyword = self$1._opts.keywords[name];
			self$1.addKeyword(name, keyword);
		}
	}
	function checkUnique(self$1, id$1) {
		if (self$1._schemas[id$1] || self$1._refs[id$1]) throw Error("schema with key or id \"" + id$1 + "\" already exists");
	}
	function getMetaSchemaOptions(self$1) {
		for (var metaOpts = util.copy(self$1._opts), i = 0; i < META_IGNORE_OPTIONS.length; i++) delete metaOpts[META_IGNORE_OPTIONS[i]];
		return metaOpts;
	}
	function setLogger(self$1) {
		var logger = self$1._opts.logger;
		if (logger === !1) self$1.logger = {
			log: noop,
			warn: noop,
			error: noop
		};
	...
	default: {}
}, import_ajv = /* @__PURE__ */ __toESM(require_ajv());
...
var require_assert = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	module.exports = function(val, options) {
		...
		if (type$1 === "number" && isFinite(val)) return options.long ? fmtLong(val) : fmtShort(val);
	...
	function setup(env) {
		...
		function createDebug(namespace) {
			...
			function debug(...args) {
				...
				let self$1 = debug, curr = Number(/* @__PURE__ */ new Date());
				self$1.diff = curr - (prevTime || curr), self$1.prev = prevTime, self$1.curr = curr, prevTime = curr, args[0] = createDebug.coerce(args[0]), typeof args[0] != "string" && args.unshift("%O");
				let index$1 = 0;
				args[0] = args[0].replace(/%([a-zA-Z%])/g, (match, format) => {
					...
					if (typeof formatter == "function") {
						...
						match = formatter.call(self$1, val), args.splice(index$1, 1), index$1--;
					}
					return match;
				}), createDebug.formatArgs.call(self$1, args), (self$1.log || createDebug.log).apply(self$1, args);
			}
	...
	function formatArgs(args) {
		...
		let index$1 = 0, lastC = 0;
		args[0].replace(/%[a-zA-Z%]/g, (match) => {
			match !== "%%" && (index$1++, match === "%c" && (lastC = index$1));
	...
	module.exports = {
		...
		dumpDot: debug.enabled ? function(codePath) {
			...
			for (let id$1 in traceMap) {
				let segment = traceMap[id$1];
				text += `${id$1}[`, segment.reachable ? text += "label=\"" : text += "style=\"rounded,dashed,filled\",fillcolor=\"#FF9800\",label=\"<<unreachable>>\\n", segment.internal.nodes.length > 0 ? text += segment.internal.nodes.join("\\n") : text += "????", text += "\"];\n";
			}
			text += `${arrows}\n`, text += "}", debug("DOT", text);
		} : debug,
		makeDotArrows(codePath, traceMap) {
			...
			for (; stack.length > 0;) {
				let item = stack.pop(), segment = item[0], index$1 = item[1];
				if (done[segment.id] && index$1 === 0) continue;
				done[segment.id] = segment;
				let nextSegment = segment.allNextSegments[index$1];
				nextSegment && (lastId === segment.id ? text += `->${nextSegment.id}` : text += `;\n${segment.id}->${nextSegment.id}`, lastId = nextSegment.id, stack.unshift([segment, 1 + index$1]), stack.push([nextSegment, 0]));
			}
	...
	module.exports = class CodePathSegment {
		constructor(id$1, allPrevSegments, reachable) {
			/* c8 ignore start */
			this.id = id$1, this.nextSegments = [], this.prevSegments = allPrevSegments.filter(isReachable), this.allNextSegments = [], this.allPrevSegments = allPrevSegments, this.reachable = reachable, Object.defineProperty(this, "internal", { value: {
				used: !1,
				loopedPrevSegments: []
		...
		static newRoot(id$1) {
			return new CodePathSegment(id$1, [], !0);
		}
		static newNext(id$1, allPrevSegments) {
			return new CodePathSegment(id$1, CodePathSegment.flattenUnusedSegments(allPrevSegments), allPrevSegments.some(isReachable));
		}
		static newUnreachable(id$1, allPrevSegments) {
			let segment = new CodePathSegment(id$1, CodePathSegment.flattenUnusedSegments(allPrevSegments), !1);
			return CodePathSegment.markUsed(segment), segment;
		}
		static newDisconnected(id$1, allPrevSegments) {
			return new CodePathSegment(id$1, [], allPrevSegments.some(isReachable));
		}
	...
	var BreakContext = class {
		...
		constructor(upperContext, type$1, label, breakContext) {
			this.upper = upperContext, this.type = type$1, this.label = label, this.brokenForkContext = breakContext.brokenForkContext;
		}
	...
	var CodePath = class {
		constructor({ id: id$1, origin, upper, onLooped }) {
			this.id = id$1, this.origin = origin, this.upper = upper, this.childCodePaths = [], Object.defineProperty(this, "internal", { value: new CodePathState(new IdGenerator(`${id$1}_`), onLooped) }), upper && upper.childCodePaths.push(this);
		}
		...
		traverseSegments(optionsOrCallback, callback) {
			...
			let startSegment = resolvedOptions.first || this.internal.initialSegment, lastSegment = resolvedOptions.last, record, index$1, end, segment = null, visited = /* @__PURE__ */ new Set(), stack = [[startSegment, 0]], skipped = /* @__PURE__ */ new Set(), broken = !1, controller = {
				skip() {
					skipped.add(segment);
				},
				break() {
			...
			for (; stack.length > 0;) {
				if (record = stack.at(-1), segment = record[0], index$1 = record[1], index$1 === 0) {
				...
				end = segment.nextSegments.length - 1, index$1 < end ? (record[1] += 1, stack.push([segment.nextSegments[index$1], 0])) : index$1 === end ? (record[0] = segment.nextSegments[index$1], record[1] = 0) : stack.pop();
			}
		}
	};
	...
	function isPropertyDefinitionValue(node) {
		...
		return parent$1 && parent$1.type === "PropertyDefinition" && parent$1.value === node;
	}
	...
	function isForkingByTrueOrFalse(node) {
		...
		switch (parent$1.type) {
			...
			case "WhileStatement":
			case "DoWhileStatement":
			case "ForStatement": return parent$1.test === node;
			case "LogicalExpression": return isHandledLogicalOperator(parent$1.operator);
			case "AssignmentExpression": return isLogicalAssignmentOperator(parent$1.operator);
			default: return !1;
		}
	}
	...
	function isIdentifierReference(node) {
		...
		switch (parent$1.type) {
			...
			case "VariableDeclarator": return parent$1.id !== node;
			case "Property":
			case "PropertyDefinition":
			case "MethodDefinition": return parent$1.key !== node || parent$1.computed || parent$1.shorthand;
			case "AssignmentPattern": return parent$1.key !== node;
			default: return !0;
		}
	}
	...
	function preprocess(analyzer, node) {
		let codePath = analyzer.codePath, state = CodePath.getState(codePath), parent$1 = node.parent;
		switch (parent$1.type) {
			case "CallExpression":
				parent$1.optional === !0 && parent$1.arguments.length >= 1 && parent$1.arguments[0] === node && state.makeOptionalRight();
				break;
			case "MemberExpression":
				parent$1.optional === !0 && parent$1.property === node && state.makeOptionalRight();
				break;
			case "LogicalExpression":
				parent$1.right === node && isHandledLogicalOperator(parent$1.operator) && state.makeLogicalRight();
				break;
			case "AssignmentExpression":
				parent$1.right === node && isLogicalAssignmentOperator(parent$1.operator) && state.makeLogicalRight();
				break;
			case "ConditionalExpression":
			case "IfStatement":
				parent$1.consequent === node ? state.makeIfConsequent() : parent$1.alternate === node && state.makeIfAlternate();
				break;
			case "SwitchCase":
				parent$1.consequent[0] === node && state.makeSwitchCaseBody(!1, !parent$1.test);
				break;
			case "TryStatement":
				parent$1.handler === node ? state.makeCatchBlock() : parent$1.finalizer === node && state.makeFinallyBlock();
				break;
			case "WhileStatement":
				parent$1.test === node ? state.makeWhileTest(getBooleanValueIfSimpleConstant(node)) : (assert(parent$1.body === node), state.makeWhileBody());
				break;
			case "DoWhileStatement":
				parent$1.body === node ? state.makeDoWhileBody() : (assert(parent$1.test === node), state.makeDoWhileTest(getBooleanValueIfSimpleConstant(node)));
				break;
			case "ForStatement":
				parent$1.test === node ? state.makeForTest(getBooleanValueIfSimpleConstant(node)) : parent$1.update === node ? state.makeForUpdate() : parent$1.body === node && state.makeForBody();
				break;
			case "ForInStatement":
			case "ForOfStatement":
				parent$1.left === node ? state.makeForInOfLeft() : parent$1.right === node ? state.makeForInOfRight() : (assert(parent$1.body === node), state.makeForInOfBody());
				break;
			case "AssignmentPattern":
				parent$1.right === node && (state.pushForkContext(), state.forkBypassPath(), state.forkPath());
				break;
			default: break;
		}
	}
	function processCodePathToEnter(analyzer, node) {
		let codePath = analyzer.codePath, state = codePath && CodePath.getState(codePath), parent$1 = node.parent;
		...
		switch (isPropertyDefinitionValue(node) && startCodePath("class-field-initializer"), node.type) {
			...
			case "TryStatement":
				...
				break;
			case "SwitchCase":
				parent$1.discriminant !== node && parent$1.cases[0] !== node && state.forkPath();
				break;
			case "WhileStatement":
			case "DoWhileStatement":
			case "ForStatement":
	...
	module.exports = class Traverser {
		...
		_traverse(node, parent$1) {
			if (isNode(node)) {
				if (this._current = node, this._skipped = !1, this._enter(node, parent$1), !this._skipped && !this._broken) {
				...
				this._broken || this._leave(node, parent$1), this._current = parent$1;
			}
		}
		static getKeys(node) {
...
})), require_Func = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	apply = curry$(function(f, list) {
		...
		return function() {
			...
			return args = res$, key = function() {
				var i$$1, ref$, len$, results$ = [];
				for (i$$1 = 0, len$ = (ref$ = args).length; i$$1 < len$; ++i$$1) arg = ref$[i$$1], results$.push(arg + toString$.call(arg).slice(8, -1));
				return results$;
			}().join(""), memo[key] = key in memo ? memo[key] : f.apply(null, args);
		};
	}, module.exports = {
		curry,
		flip,
	...
	function curry$(f, bound) {
		var context, _curry = function(args) {
			return f.length > 1 ? function() {
				...
				return context = bound && context || this, params.push.apply(params, arguments) < f.length && arguments.length ? _curry.call(context, params) : f.apply(context, params);
			} : f;
		};
		return _curry();
	}
})), require_List = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	each = curry$(function(f, xs) {
		var i$, len$, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) x = xs[i$], f(x);
		return xs;
	}), map = curry$(function(f, xs) {
		var i$, len$, x, results$ = [];
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) x = xs[i$], results$.push(f(x));
		return results$;
	}), compact = function(xs) {
		var i$, len$, x, results$ = [];
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) x = xs[i$], x && results$.push(x);
		return results$;
	}, filter = curry$(function(f, xs) {
		var i$, len$, x, results$ = [];
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) x = xs[i$], f(x) && results$.push(x);
		return results$;
		...
		var i = elemIndex(el, xs), x$ = xs.slice();
		return i != null && x$.splice(i, 1), x$;
	}), partition = curry$(function(f, xs) {
		var passed = [], failed = [], i$, len$, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) x = xs[i$], (f(x) ? passed : failed).push(x);
		return [passed, failed];
	}), find = curry$(function(f, xs) {
		var i$, len$, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) if (x = xs[i$], f(x)) return x;
	}), head = first = function(xs) {
		return xs[0];
	}, tail = function(xs) {
		if (xs.length) return xs.slice(1);
	}, last = function(xs) {
		return xs[xs.length - 1];
	}, initial = function(xs) {
		if (xs.length) return xs.slice(0, -1);
	}, empty = function(xs) {
		return !xs.length;
	}, reverse = function(xs) {
		...
		var result = [], i$, len$, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) x = xs[i$], in$(x, result) || result.push(x);
		return result;
	}, uniqueBy = curry$(function(f, xs) {
		var seen, i$, len$, x, val, results$ = [];
		for (seen = [], i$ = 0, len$ = xs.length; i$ < len$; ++i$) x = xs[i$], val = f(x), !in$(val, seen) && (seen.push(val), results$.push(x));
		return results$;
	}), fold = foldl = curry$(function(f, memo, xs) {
		var i$, len$, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) x = xs[i$], memo = f(memo, x);
		return memo;
	}), fold1 = foldl1 = curry$(function(f, xs) {
		return fold(f, xs[0], xs.slice(1));
	}), foldr = curry$(function(f, memo, xs) {
		var i$, x;
		for (i$ = xs.length - 1; i$ >= 0; --i$) x = xs[i$], memo = f(x, memo);
		return memo;
	}), foldr1 = curry$(function(f, xs) {
		return foldr(f, xs[xs.length - 1], xs.slice(0, -1));
	}), unfoldr = curry$(function(f, b) {
		for (var result = [], x = b, that; (that = f(x)) != null;) result.push(that[0]), x = that[1];
		return result;
	}), concat = function(xss) {
		...
		return [].concat.apply([], function() {
			...
			for (i$ = 0, len$ = (ref$ = xs).length; i$ < len$; ++i$) x = ref$[i$], results$.push(f(x));
			return results$;
		}());
	}), flatten = function(xs) {
		var x;
		return [].concat.apply([], function() {
			var i$, ref$, len$, results$ = [];
			for (i$ = 0, len$ = (ref$ = xs).length; i$ < len$; ++i$) x = ref$[i$], toString$.call(x).slice(8, -1) === "Array" ? results$.push(flatten(x)) : results$.push(x);
			return results$;
		}());
		...
		outer: for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) {
			for (x = xs[i$], j$ = 0, len1$ = yss.length; j$ < len1$; ++j$) if (ys = yss[j$], in$(x, ys)) continue outer;
			results.push(x);
		}
		return results;
		...
		for (i$ = 0, to$ = arguments.length; i$ < to$; ++i$) res$.push(arguments[i$]);
		for (xss = res$, results = [], i$ = 0, len$ = xss.length; i$ < len$; ++i$) for (xs = xss[i$], j$ = 0, len1$ = xs.length; j$ < len1$; ++j$) x = xs[j$], in$(x, results) || results.push(x);
		return results;
	}, countBy = curry$(function(f, xs) {
		var results = {}, i$, len$, x, key;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) x = xs[i$], key = f(x), key in results ? results[key] += 1 : results[key] = 1;
		return results;
	}), groupBy = curry$(function(f, xs) {
		var results = {}, i$, len$, x, key;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) x = xs[i$], key = f(x), key in results ? results[key].push(x) : results[key] = [x];
		return results;
	}), andList = function(xs) {
		var i$, len$, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) if (x = xs[i$], !x) return !1;
		return !0;
	}, orList = function(xs) {
		var i$, len$, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) if (x = xs[i$], x) return !0;
		return !1;
	}, any = curry$(function(f, xs) {
		var i$, len$, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) if (x = xs[i$], f(x)) return !0;
		return !1;
	}), all = curry$(function(f, xs) {
		var i$, len$, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) if (x = xs[i$], !f(x)) return !1;
		return !0;
	}), sort = function(xs) {
		...
		return xs.concat().sort(function(x, y) {
			return f(x) > f(y) ? 1 : f(x) < f(y) ? -1 : 0;
		});
	}), sum = function(xs) {
		var result = 0, i$, len$, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) x = xs[i$], result += x;
		return result;
	}, product = function(xs) {
		var result = 1, i$, len$, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) x = xs[i$], result *= x;
		return result;
	}, mean = average = function(xs) {
		var sum = 0, i$, len$, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) x = xs[i$], sum += x;
		return sum / xs.length;
	}, maximum = function(xs) {
		var max = xs[0], i$, ref$, len$, x;
		for (i$ = 0, len$ = (ref$ = xs.slice(1)).length; i$ < len$; ++i$) x = ref$[i$], x > max && (max = x);
		return max;
	}, minimum = function(xs) {
		var min = xs[0], i$, ref$, len$, x;
		for (i$ = 0, len$ = (ref$ = xs.slice(1)).length; i$ < len$; ++i$) x = ref$[i$], x < min && (min = x);
		return min;
	}, maximumBy = curry$(function(f, xs) {
		var max = xs[0], i$, ref$, len$, x;
		for (i$ = 0, len$ = (ref$ = xs.slice(1)).length; i$ < len$; ++i$) x = ref$[i$], f(x) > f(max) && (max = x);
		return max;
	}), minimumBy = curry$(function(f, xs) {
		var min = xs[0], i$, ref$, len$, x;
		for (i$ = 0, len$ = (ref$ = xs.slice(1)).length; i$ < len$; ++i$) x = ref$[i$], f(x) < f(min) && (min = x);
		return min;
	}), scan = scanl = curry$(function(f, memo, xs) {
		var last = memo, x;
		return [memo].concat(function() {
			var i$, ref$, len$, results$ = [];
			for (i$ = 0, len$ = (ref$ = xs).length; i$ < len$; ++i$) x = ref$[i$], results$.push(last = f(last, x));
			return results$;
		}());
	}), scan1 = scanl1 = curry$(function(f, xs) {
		if (xs.length) return scan(f, xs[0], xs.slice(1));
	}), scanr = curry$(function(f, memo, xs) {
		return xs = xs.concat().reverse(), scan(f, memo, xs).reverse();
	}), scanr1 = curry$(function(f, xs) {
		if (xs.length) return xs = xs.concat().reverse(), scan(f, xs[0], xs.slice(1)).reverse();
	}), slice = curry$(function(x, y, xs) {
		return xs.slice(x, y);
	}), take = curry$(function(n, xs) {
		return n <= 0 ? xs.slice(0, 0) : xs.slice(0, n);
	}), drop = curry$(function(n, xs) {
		return n <= 0 ? xs : xs.slice(n);
	}), splitAt = curry$(function(n, xs) {
		return [take(n, xs), drop(n, xs)];
	}), takeWhile = curry$(function(p, xs) {
		var len = xs.length, i;
		if (!len) return xs;
		for (i = 0; i < len && p(xs[i]);) i += 1;
		return xs.slice(0, i);
		...
		var result = [], len = ys.length, i$, len$, i, x;
		for (i$ = 0, len$ = xs.length; i$ < len$ && (i = i$, x = xs[i$], i !== len); ++i$) result.push([x, ys[i]]);
		return result;
	}), zipWith = curry$(function(f, xs, ys) {
		var result = [], len = ys.length, i$, len$, i, x;
		for (i$ = 0, len$ = xs.length; i$ < len$ && (i = i$, x = xs[i$], i !== len); ++i$) result.push(f(x, ys[i]));
		return result;
	}), zipAll = function() {
		var xss, res$, i$, to$, minLength, len$, xs, ref$, i, lresult$, j$, results$ = [];
		for (res$ = [], i$ = 0, to$ = arguments.length; i$ < to$; ++i$) res$.push(arguments[i$]);
		for (xss = res$, minLength = void 0, i$ = 0, len$ = xss.length; i$ < len$; ++i$) xs = xss[i$], minLength <= (ref$ = xs.length) || (minLength = ref$);
		for (i$ = 0; i$ < minLength; ++i$) {
			for (i = i$, lresult$ = [], j$ = 0, len$ = xss.length; j$ < len$; ++j$) xs = xss[j$], lresult$.push(xs[i]);
			results$.push(lresult$);
		}
		return results$;
	}, zipAllWith = function(f) {
		var xss, res$, i$, to$, minLength, len$, xs, ref$, i, results$ = [];
		for (res$ = [], i$ = 1, to$ = arguments.length; i$ < to$; ++i$) res$.push(arguments[i$]);
		for (xss = res$, minLength = void 0, i$ = 0, len$ = xss.length; i$ < len$; ++i$) xs = xss[i$], minLength <= (ref$ = xs.length) || (minLength = ref$);
		for (i$ = 0; i$ < minLength; ++i$) i = i$, results$.push(f.apply(null, fn$()));
		return results$;
		function fn$() {
			var i$$1, ref$$1, len$$1, results$$1 = [];
			for (i$$1 = 0, len$$1 = (ref$$1 = xss).length; i$$1 < len$$1; ++i$$1) xs = ref$$1[i$$1], results$$1.push(xs[i]);
			return results$$1;
		}
	}, at = curry$(function(n, xs) {
		return n < 0 ? xs[xs.length + n] : xs[n];
	}), elemIndex = curry$(function(el, xs) {
		var i$, len$, i, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) if (i = i$, x = xs[i$], x === el) return i;
	}), elemIndices = curry$(function(el, xs) {
		var i$, len$, i, x, results$ = [];
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) i = i$, x = xs[i$], x === el && results$.push(i);
		return results$;
	}), findIndex = curry$(function(f, xs) {
		var i$, len$, i, x;
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) if (i = i$, x = xs[i$], f(x)) return i;
	}), findIndices = curry$(function(f, xs) {
		var i$, len$, i, x, results$ = [];
		for (i$ = 0, len$ = xs.length; i$ < len$; ++i$) i = i$, x = xs[i$], f(x) && results$.push(i);
		return results$;
	}), module.exports = {
		each,
		map,
	...
	function in$(x, xs) {
		for (var i = -1, l = xs.length >>> 0; ++i < l;) if (x === xs[i]) return !0;
		return !1;
	}
	function compose$() {
		var functions = arguments;
		return function() {
			var i, result = functions[0].apply(this, arguments);
...
})), require_Obj = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var values = function(object) {
		var i$, x, results$ = [];
		for (i$ in object) x = object[i$], results$.push(x);
		return results$;
		...
		for (i$ = 0, len$ = object.length; i$ < len$; ++i$) x = object[i$], resultObj$[x[0]] = x[1];
		return resultObj$;
	}, objToPairs = function(object) {
		var key, value, results$ = [];
		for (key in object) value = object[key], results$.push([key, value]);
		return results$;
	}, listsToObj = curry$(function(keys, values) {
		var i$, len$, i, key, resultObj$ = {};
		for (i$ = 0, len$ = keys.length; i$ < len$; ++i$) i = i$, key = keys[i$], resultObj$[key] = values[i];
		return resultObj$;
	}), objToLists = function(object) {
		var keys = [], values = [], key, value;
		for (key in object) value = object[key], keys.push(key), values.push(value);
		return [keys, values];
	}, empty = function(object) {
		for (var x in object) return !1;
		return !0;
	}, each = curry$(function(f, object) {
		var i$, x;
		for (i$ in object) x = object[i$], f(x);
		return object;
	}), map = curry$(function(f, object) {
		var k, x, resultObj$ = {};
		for (k in object) x = object[k], resultObj$[k] = f(x);
		return resultObj$;
	}), compact = function(object) {
		var k, x, resultObj$ = {};
		for (k in object) x = object[k], x && (resultObj$[k] = x);
		return resultObj$;
	};
	module.exports = {
		values,
		...
		filter: curry$(function(f, object) {
			var k, x, resultObj$ = {};
			for (k in object) x = object[k], f(x) && (resultObj$[k] = x);
			return resultObj$;
		}),
		compact,
		...
		partition: curry$(function(f, object) {
			...
			for (k in object) x = object[k], (f(x) ? passed : failed)[k] = x;
			return [passed, failed];
		}),
		find: curry$(function(f, object) {
			var i$, x;
			for (i$ in object) if (x = object[i$], f(x)) return x;
		})
	};
	function curry$(f, bound) {
		var context, _curry = function(args) {
	...
	var split = curry$(function(sep, str) {
		...
		return str.replace(/([^-A-Z])([A-Z]+)/g, function(arg$, lower, upper) {
			...
			return upper.length > 1 ? upper + "-" : upper.toLowerCase();
		});
	};
	module.exports = {
		split,
...
})), require_Num = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var max = curry$(function(x$, y$) {
		...
		var z;
		for (x = Math.abs(x), y = Math.abs(y); y !== 0;) z = x % y, x = y, y = z;
		return x;
	});
	module.exports = {
		max,
...
})), require_lib$2 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var Func, List, Obj, Str, Num, id, isType, replicate, prelude, toString$ = {}.toString;
	Func = require_Func(), List = require_List(), Obj = require_Obj(), Str = require_Str(), Num = require_Num(), id = function(x) {
		return x;
	}, isType = curry$(function(type$1, x) {
		return toString$.call(x).slice(8, -1) === type$1;
...
})), require_parse_string = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function() {
		var reject = require_lib$2().reject, special, tokenRegex;
		function consumeOp(tokens$1, op) {
			if (tokens$1[0] === op) return tokens$1.shift();
			throw Error("Expected '" + op + "', but got '" + tokens$1[0] + "' instead in " + JSON.stringify(tokens$1) + ".");
		}
		function maybeConsumeOp(tokens$1, op) {
			if (tokens$1[0] === op) return tokens$1.shift();
		}
		function consumeList(tokens$1, arg$, hasDelimiters) {
			var open = arg$[0], close = arg$[1], result, untilTest;
			for (hasDelimiters && consumeOp(tokens$1, open), result = [], untilTest = "," + (hasDelimiters ? close : ""); tokens$1.length && hasDelimiters && tokens$1[0] !== close;) result.push(consumeElement(tokens$1, untilTest)), maybeConsumeOp(tokens$1, ",");
			return hasDelimiters && consumeOp(tokens$1, close), result;
		}
		function consumeArray(tokens$1, hasDelimiters) {
			return consumeList(tokens$1, ["[", "]"], hasDelimiters);
		}
		function consumeTuple(tokens$1, hasDelimiters) {
			return consumeList(tokens$1, ["(", ")"], hasDelimiters);
		}
		function consumeFields(tokens$1, hasDelimiters) {
			var result, untilTest, key;
			for (hasDelimiters && consumeOp(tokens$1, "{"), result = {}, untilTest = "," + (hasDelimiters ? "}" : ""); tokens$1.length && (!hasDelimiters || tokens$1[0] !== "}");) key = consumeValue(tokens$1, ":"), consumeOp(tokens$1, ":"), result[key] = consumeElement(tokens$1, untilTest), maybeConsumeOp(tokens$1, ",");
			return hasDelimiters && consumeOp(tokens$1, "}"), result;
		}
		function consumeValue(tokens$1, untilTest) {
			var out;
			for (untilTest ??= "", out = ""; tokens$1.length && untilTest.indexOf(tokens$1[0]) === -1;) out += tokens$1.shift();
			return out;
		}
		function consumeElement(tokens$1, untilTest) {
			switch (tokens$1[0]) {
				case "[": return consumeArray(tokens$1, !0);
				case "(": return consumeTuple(tokens$1, !0);
				case "{": return consumeFields(tokens$1, !0);
				default: return consumeValue(tokens$1, untilTest);
			}
		}
		function consumeTopLevel(tokens$1, types, options) {
			var ref$ = types[0], type$1 = ref$.type, structure = ref$.structure, origTokens = tokens$1.concat(), result, finalResult, x$, y$;
			return !options.explicit && types.length === 1 && (!type$1 && structure || type$1 === "Array" || type$1 === "Object") ? (result = structure === "array" || type$1 === "Array" ? consumeArray(tokens$1, tokens$1[0] === "[") : structure === "tuple" ? consumeTuple(tokens$1, tokens$1[0] === "(") : consumeFields(tokens$1, tokens$1[0] === "{"), finalResult = tokens$1.length ? consumeElement(structure === "array" || type$1 === "Array" ? (x$ = origTokens, x$.unshift("["), x$.push("]"), x$) : (y$ = origTokens, y$.unshift("("), y$.push(")"), y$)) : result) : finalResult = consumeElement(tokens$1), finalResult;
		}
		special = "\\[\\]\\(\\)}{:,", tokenRegex = RegExp("(\"(?:\\\\\"|[^\"])*\")|('(?:\\\\'|[^'])*')|(/(?:\\\\/|[^/])*/[a-zA-Z]*)|(#.*#)|([" + special + "])|([^\\s" + special + "](?:\\s*[^\\s" + special + "]+)*)|\\s*"), module.exports = function(types, string, options) {
			var tokens$1, node;
			if (options ??= {}, !options.explicit && types.length === 1 && types[0].type === "String") return string;
			if (tokens$1 = reject(not$, string.split(tokenRegex)), node = consumeTopLevel(tokens$1, types, options), !node) throw Error("Error parsing '" + string + "'.");
			return node;
		};
		function not$(x) {
			return !x;
...
})), require_parse_type = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function() {
		var identifierRegex = /[\$\w]+/, tokenRegex;
		function peek(tokens$1) {
			var token = tokens$1[0];
			if (token == null) throw Error("Unexpected end of input.");
			return token;
		}
		function consumeIdent(tokens$1) {
			var token = peek(tokens$1);
			if (!identifierRegex.test(token)) throw Error("Expected text, got '" + token + "' instead.");
			return tokens$1.shift();
		}
		function consumeOp(tokens$1, op) {
			var token = peek(tokens$1);
			if (token !== op) throw Error("Expected '" + op + "', got '" + token + "' instead.");
			return tokens$1.shift();
		}
		function maybeConsumeOp(tokens$1, op) {
			return tokens$1[0] === op ? tokens$1.shift() : null;
		}
		function consumeArray(tokens$1) {
			var types;
			if (consumeOp(tokens$1, "["), peek(tokens$1) === "]") throw Error("Must specify type of Array - eg. [Type], got [] instead.");
			return types = consumeTypes(tokens$1), consumeOp(tokens$1, "]"), {
				structure: "array",
				of: types
			};
		}
		function consumeTuple(tokens$1) {
			var components = [];
			if (consumeOp(tokens$1, "("), peek(tokens$1) === ")") throw Error("Tuple must be of at least length 1 - eg. (Type), got () instead.");
			for (; components.push(consumeTypes(tokens$1)), maybeConsumeOp(tokens$1, ","), peek(tokens$1) !== ")";);
			return consumeOp(tokens$1, ")"), {
				structure: "tuple",
				of: components
			};
		}
		function consumeFields(tokens$1) {
			var fields = {}, subset, ref$, key, types;
			for (consumeOp(tokens$1, "{"), subset = !1;;) {
				if (maybeConsumeOp(tokens$1, "...")) {
					subset = !0;
					break;
				}
				if (ref$ = consumeField(tokens$1), key = ref$[0], types = ref$[1], fields[key] = types, maybeConsumeOp(tokens$1, ","), peek(tokens$1) === "}") break;
			}
			return consumeOp(tokens$1, "}"), {
				structure: "fields",
				of: fields,
		...
		function consumeField(tokens$1) {
			var key = consumeIdent(tokens$1), types;
			return consumeOp(tokens$1, ":"), types = consumeTypes(tokens$1), [key, types];
		}
		function maybeConsumeStructure(tokens$1) {
			switch (tokens$1[0]) {
				case "[": return consumeArray(tokens$1);
				case "(": return consumeTuple(tokens$1);
				case "{": return consumeFields(tokens$1);
			}
		}
		function consumeType(tokens$1) {
			var token = peek(tokens$1), wildcard = token === "*", type$1, structure;
			if (wildcard || identifierRegex.test(token)) return type$1 = wildcard ? consumeOp(tokens$1, "*") : consumeIdent(tokens$1), structure = maybeConsumeStructure(tokens$1), structure ? (structure.type = type$1, structure) : { type: type$1 };
			if (structure = maybeConsumeStructure(tokens$1), !structure) throw Error("Unexpected character: " + token);
			return structure;
		}
		function consumeTypes(tokens$1) {
			var lookahead, types, typesSoFar, typeObj, type$1, structure;
			if (peek(tokens$1) === "::") throw Error("No comment before comment separator '::' found.");
			for (lookahead = tokens$1[1], lookahead != null && lookahead === "::" && (tokens$1.shift(), tokens$1.shift()), types = [], typesSoFar = {}, peek(tokens$1) === "Maybe" && (tokens$1.shift(), types = [{ type: "Undefined" }, { type: "Null" }], typesSoFar = {
				Undefined: !0,
				Null: !0
			}); typeObj = consumeType(tokens$1), type$1 = typeObj.type, structure = typeObj.structure, typesSoFar[type$1] || types.push(typeObj), structure ?? (typesSoFar[type$1] = !0), maybeConsumeOp(tokens$1, "|"););
			return types;
		}
		tokenRegex = RegExp("\\.\\.\\.|::|->|" + identifierRegex.source + "|\\S", "g"), module.exports = function(input) {
			var tokens$1, e;
			if (!input.length) throw Error("No type specified.");
			if (tokens$1 = input.match(tokenRegex) || [], in$("->", tokens$1)) throw Error("Function types are not supported. To validate that something is a function, you may use 'Function'.");
			try {
				return consumeTypes(tokens$1);
			} catch (e$) {
				throw e = e$, Error(e.message + " - Remaining tokens: " + JSON.stringify(tokens$1) + " - Initial input: '" + input + "'");
			}
		};
		function in$(x, xs) {
			for (var i = -1, l = xs.length >>> 0; ++i < l;) if (x === xs[i]) return !0;
...
})), require_check = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function() {
		var ref$, any, all, isItNaN, types, defaultType, toString$ = {}.toString;
		ref$ = require_lib$2(), any = ref$.any, all = ref$.all, isItNaN = ref$.isItNaN, types = {
			Number: {
				typeOf: "Number",
		...
		function checkArray(input, type$1, options) {
			return all(function(it) {
				return checkMultiple(it, type$1.of, options);
			}, input);
		}
		function checkTuple(input, type$1, options) {
			var i = 0, i$, ref$$1, len$, types$1;
			for (i$ = 0, len$ = (ref$$1 = type$1.of).length; i$ < len$; ++i$) {
				if (types$1 = ref$$1[i$], !checkMultiple(input[i], types$1, options)) return !1;
				i++;
			}
			return input.length <= i;
		}
		function checkFields(input, type$1, options) {
			var inputKeys = {}, numInputKeys = 0, k, numOfKeys, key, ref$$1, types$1;
			for (k in input) inputKeys[k] = !0, numInputKeys++;
			for (key in numOfKeys = 0, ref$$1 = type$1.of) {
				if (types$1 = ref$$1[key], !checkMultiple(input[key], types$1, options)) return !1;
				inputKeys[key] && numOfKeys++;
			}
			return type$1.subset || numInputKeys === numOfKeys;
		}
		function checkStructure(input, type$1, options) {
			if (!(input instanceof Object)) return !1;
			switch (type$1.structure) {
				case "fields": return checkFields(input, type$1, options);
				case "array": return checkArray(input, type$1, options);
				case "tuple": return checkTuple(input, type$1, options);
			}
		}
		function check(input, typeObj, options) {
			var type$1 = typeObj.type, structure = typeObj.structure, setting, that;
			if (type$1) return type$1 === "*" ? !0 : (setting = options.customTypes[type$1] || types[type$1], setting ? (setting.typeOf === void 0 || setting.typeOf === toString$.call(input).slice(8, -1)) && setting.validate(input) : type$1 === toString$.call(input).slice(8, -1) && (!structure || checkStructure(input, typeObj, options)));
			if (structure) return (that = defaultType[structure]) && that !== toString$.call(input).slice(8, -1) ? !1 : checkStructure(input, typeObj, options);
			throw Error("No type defined. Input: " + input + ".");
		}
		function checkMultiple(input, types$1, options) {
			if (toString$.call(types$1).slice(8, -1) !== "Array") throw Error("Types must be in an array. Input: " + input + ".");
			return any(function(it) {
				return check(input, it, options);
			}, types$1);
		}
		module.exports = function(parsedType, input, options) {
			return options ??= {}, options.customTypes ??= {}, checkMultiple(input, parsedType, options);
		};
	}).call(exports);
})), require_lib$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function() {
		var VERSION = "0.4.0", parseType = require_parse_type(), parsedTypeCheck = require_check(), typeCheck = function(type$1, input, options) {
			return parsedTypeCheck(parseType(type$1), input, options);
		};
		module.exports = {
			VERSION,
			typeCheck,
...
})), require_cast = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function() {
		var parsedTypeCheck, types, toString$ = {}.toString;
		parsedTypeCheck = require_lib$1().parsedTypeCheck, types = {
			"*": function(value, options) {
				switch (toString$.call(value).slice(8, -1)) {
					case "Array": return typeCast(value, { type: "Array" }, options);
					case "Object": return typeCast(value, { type: "Object" }, options);
					default: return {
						type: "Just",
						value: typesCast(value, [
							...
							{ type: "Object" },
							{ type: "String" }
						], (options.explicit = !0, options))
					};
				}
			},
			Undefined: function(it) {
				return it === "undefined" || it === void 0 ? {
					type: "Just",
					value: void 0
				} : { type: "Nothing" };
			},
			...
			Date: function(value, options) {
				var that;
				return (that = /^\#([\s\S]*)\#$/.exec(value)) ? {
					type: "Just",
					value: new Date(+that[1] || that[1])
				} : options.explicit ? { type: "Nothing" } : {
					type: "Just",
			...
			RegExp: function(value, options) {
				var that;
				return (that = /^\/([\s\S]*)\/([gimy]*)$/.exec(value)) ? {
					type: "Just",
			...
			String: function(it) {
				var replace, that;
				return toString$.call(it).slice(8, -1) === "String" ? (replace = function(value, quote) {
		...
		function castArray(node, type$1, options) {
			var typeOf, element;
			return toString$.call(node).slice(8, -1) === "Array" ? (typeOf = type$1.of, {
				type: "Just",
				value: function() {
					var i$, ref$, len$, results$ = [];
					for (i$ = 0, len$ = (ref$ = node).length; i$ < len$; ++i$) element = ref$[i$], results$.push(typesCast(element, typeOf, options));
					return results$;
				}()
			}) : { type: "Nothing" };
		}
		function castTuple(node, type$1, options) {
			var result, i, i$, ref$, len$, types$1, cast;
			if (toString$.call(node).slice(8, -1) !== "Array") return { type: "Nothing" };
			for (result = [], i = 0, i$ = 0, len$ = (ref$ = type$1.of).length; i$ < len$; ++i$) types$1 = ref$[i$], cast = typesCast(node[i], types$1, options), toString$.call(cast).slice(8, -1) !== "Undefined" && result.push(cast), i++;
			return node.length <= i ? {
				type: "Just",
				value: result
			} : { type: "Nothing" };
		}
		function castFields(node, type$1, options) {
			var typeOf, key, value;
			return toString$.call(node).slice(8, -1) === "Object" ? (typeOf = type$1.of, {
				type: "Just",
				value: function() {
					var ref$, resultObj$ = {};
					for (key in ref$ = node) value = ref$[key], resultObj$[typesCast(key, [{ type: "String" }], options)] = typesCast(value, typeOf[key] || [{ type: "*" }], options);
					return resultObj$;
				}()
			}) : { type: "Nothing" };
		}
		function typeCast(node, typeObj, options) {
			var type$1 = typeObj.type, structure = typeObj.structure, castFunc;
			if (type$1) {
				if (castFunc = options.customTypes[type$1]?.cast || types[type$1], !castFunc) throw Error("Type not defined: " + type$1 + ".");
				return castFunc(node, options, typesCast);
			} else switch (structure) {
				case "array": return castArray(node, typeObj, options);
				case "tuple": return castTuple(node, typeObj, options);
				case "fields": return castFields(node, typeObj, options);
			}
		}
		function typesCast(node, types$1, options) {
			var i$, len$, type$1, ref$, valueType, value;
			for (i$ = 0, len$ = types$1.length; i$ < len$; ++i$) if (type$1 = types$1[i$], ref$ = typeCast(node, type$1, options), valueType = ref$.type, value = ref$.value, valueType !== "Nothing" && parsedTypeCheck([type$1], value, { customTypes: options.customTypes })) return value;
			throw Error("Value " + JSON.stringify(node) + " does not type check against " + JSON.stringify(types$1) + ".");
		}
		module.exports = function(node, types$1, options) {
			return !options.explicit && types$1.length === 1 && types$1[0].type === "String" ? node : typesCast(node, types$1, options);
		};
	}).call(exports);
}));
(/* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function() {
		var parseString = require_parse_string(), cast = require_cast(), parseType = require_lib$1().parseType, VERSION = "0.4.1", parsedTypeParse = function(parsedType, string, options) {
			return options ??= {}, options.explicit ??= !1, options.customTypes ??= {}, cast(parseString(parsedType, string, options), parsedType, options);
		}, parse = function(type$1, string, options) {
			return parsedTypeParse(parseType(type$1), string, options);
		};
		module.exports = {
			VERSION,
			parse,
...
var VisitNodeStep = class {
	...
	constructor({ target, phase, args }) {
		this.target = target, this.phase = phase, this.args = args;
	}
	...
	constructor({ target, args }) {
		this.target = target, this.args = args;
	}
};
const NODE_TYPE_IDS_MAP = new Map([
	...
	55,
	56
], ancestors = [], { isArray } = Array;
function walkNode(node, visitors) {
	if (node != null) if (isArray(node)) {
		let len = node.length;
...
function walkProgramWithCfg(ast$1, visitors) {
	prepareSteps(ast$1);
	let stepsLen = steps.length;
	for (let i = 0; i < stepsLen; i++) {
		let step = steps[i];
		if (step.kind === 1) {
			let node = step.target;
			if (step.phase === 1) {
				let typeId = NODE_TYPE_IDS_MAP.get(node.type), visit = visitors[typeId];
				...
				ancestors.shift();
				let enterExit = visitors[NODE_TYPE_IDS_MAP.get(node.type)];
				if (enterExit !== null) {
					let { exit } = enterExit;
					exit !== null && exit(node);
				}
			...
			let visit = visitors[NODE_TYPE_IDS_MAP.get(step.target)];
			visit !== null && visit.apply(void 0, step.args);
		}
	}
	steps.length = 0;
}
function prepareSteps(ast$1) {
	let analyzer = new import_code_path_analyzer.default({
		...
		leaveNode(node) {
			NODE_TYPE_IDS_MAP.get(node.type) >= 27 && steps.push(new VisitNodeStep({
				target: node,
				phase: 2,
				args: [node]
			}));
	...
	import_traverser.default.traverse(ast$1, {
		enter(node) {
...
function matchesSelectorClass(className, node, _ancestors) {
	let { type: type$1 } = node;
	switch (className.toLowerCase()) {
		case "statement": if (type$1.endsWith("Statement")) return !0;
		case "declaration": return type$1.endsWith("Declaration");
		case "pattern": if (type$1.endsWith("Pattern")) return !0;
		case "expression": return type$1.endsWith("Expression") || type$1.endsWith("Literal") || type$1 === "Identifier" && node.parent.type !== "MetaProperty" || type$1 === "MetaProperty";
		case "function": return type$1 === "FunctionDeclaration" || type$1 === "FunctionExpression" || type$1 === "ArrowFunctionExpression";
		default: return !1;
	}
}
	...
	if (keysLen !== 0) {
		...
		for (let i = 0; i < keysLen; i++) {
			...
			if (name !== "*") {
				...
				if (typeIds !== null) {
					for (let i$1 = 0, len = typeIds.length; i$1 < len; i$1++) {
						let typeId$1 = typeIds[i$1];
						typeId$1 < 27 ? addLeafVisitFn(typeId$1, visitProp) : addNonLeafVisitFn(typeId$1, visitProp, isExit);
					}
					continue;
				}
			}
...
function lintFile(filePath$1, bufferId, buffer$1, ruleIds, optionsIds, settingsJSON$1, globalsJSON$1) {
	try {
		lintFileImpl(filePath$1, bufferId, buffer$1, ruleIds, optionsIds, settingsJSON$1, globalsJSON$1);
		let ret = null;
...
export { ObjectSetPrototypeOf as A, ObjectAssign as C, ObjectGetPrototypeOf as D, ObjectFreeze as E, ObjectHasOwn as O, JSONStringify as S, ObjectDefineProperties as T, BUFFER_ALIGN as _, loadPlugin as a, ArrayFrom as b, getErrorMessage as c, setOptions as d, PLACEHOLDER_REGEX as f, getNodeByRangeIndex as g, getLineColumnFromOffset as h, resetStateAfterError as i, ObjectValues as j, ObjectKeys as k, DEFAULT_OPTIONS_ID as l, replacePlaceholders as m, lintFile as n, registerPlugin as o, diagnostics as p, lintFileImpl as r, registeredRules as s, buffers as t, allOptions as u, BUFFER_SIZE as v, ObjectCreate as w, ArrayIsArray as x, DATA_POINTER_POS_32 as y };
